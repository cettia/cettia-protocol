...
module.exports = function(uri, options) {
  ...
  // URI's protocol should be either `http` or `https` and cettia-transport-name param
  // should be `longpoll`.
  if ((urlObj.protocol === "http:" || urlObj.protocol === "https:")
    ...
    self.connect = function() {
      ...
      .on("response", function(res) {
        // If the response's status code is not 200, fires an error and closes the connection.
        ...
        .on("end", function() {
          ...
          // Then starts polling.
          function poll() {
            // From the second request, `cettia-transport-when` param should be `poll` and
            // `cettia-transport-id` param should be added.
            var reqOpts = url.parse(uri + "&cettia-transport-when=poll&cettia-transport-id=" +
