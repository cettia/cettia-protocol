...
module.exports = function() {
  ...
  // Since a HTTP transport consists of multiple HTTP exchanges, transport object is needed to
  // be retrieved using identifier.
  var transports = {};
  ...
  // A link between Cettia HTTP transport protocol and Node.js. `req` and `res` are expected to
  // be passed from Node.js's `http/https` module's server's `request` event.
  self.handle = function(req, res) {
    ...
    // Transports using `XDomainRequest` require CORS headers even in same-origin connection.
    res.setHeader("access-control-allow-origin", req.headers.origin || "*");
    // As `content-type` header, `text/plain` for text message and `application/octet-stream`
    // for binary message are used. But, because the latter is not said to be a "simple headers"
    // defined by CORS spec, `content-type` header should be listed as a value of
    // `access-control-allow-headers`.
    res.setHeader("access-control-allow-headers", "content-type");
    // For convenience.
    res.setHeader("access-control-allow-credentials", "true");
    switch (req.method) {
      // `OPTIONS` method is used to handle CORS preflight request. Since the client needs to
      // get `content-type` response header and it may be `application/octet-stream` which is
      // not "simple header" coined by CORS spec, browser will perform this preflight request.
      case "OPTIONS":
        // Ends the response as necessary headers are already set.
        res.end();
        break;
      // `GET` method is used to establish a channel for the server to write message to the
      // client and manage existing transports.
      case "GET":
        // `cettia-transport-when` param indicates a specific goal of `GET` request.
        switch (req.params["cettia-transport-when"]) {
          // To establish a new channel.
          case "open":
            // `cettia-transport-name` param is transport name the client uses which is either
            // `stream` or `longpoll`.
            switch (req.params["cettia-transport-name"]) {
              case "stream":
              ...
              case "longpoll":
                ...
                break;
              // For unknown transport name, responds with `501 Not Implemented`.
              default:
                res.statusCode = 501;
                res.end();
                break;
            ...
            break;
          // To inject a new HTTP request-response exchange to long polling transport.
          case "poll":
            // Finds the corresponding transport by `cettia-transport-id` param.
            var transport = transports[req.params["cettia-transport-id"]];
            if (transport) {
          ...
          case "abort":
            // Finds the corresponding transport by `cettia-transport-id` param.
            var transport = transports[req.params["cettia-transport-id"]];
            // If server detected disconnection correctly, `transport` must be null.
            if (transport) {
              // For some reason, the server couldn't detect disconnection then close connection
              // to fire `close` event to transport manually.
              transport.close();
            }
            // In case of browser, it is performed by script tag so set content-type header to
            // `text/javascript` to avoid warning.
            res.setHeader("content-type", "text/javascript; charset=utf-8");
            res.end();
            break;
          // For unknown `cettia-transport-when` param, responds with `501 Not Implemented`.
          default:
            res.statusCode = 501;
            res.end();
        }
        break;
      // `POST` method is used to establish a channel for the client to write message to the server.
      case "POST":
        ...
        req.on("end", function() {
          ...
          if (transport) {
            ...
            switch ((req.headers["content-type"] || "").toLowerCase()) {
              ...
              case "text/plain;charset=utf-8":
              case "text/plain;charset=utf8":
                // Fires `text` event by decoding the body with `utf-8` and stripping off
                // leading `data=`.
              ...
              case "application/octet-stream":
                ...
                break;
              default:
                // If the content-type header is invalid, fires an error and closes the connection.
            ...
            // If the specified transport is not found, responds with `500 Internal Server
            // Error`. It might happen if the transport is closed while reading request body.
          } else {
            res.statusCode = 500;
          }
...
function createBaseTransport(req, res) {
  // A transport object.
  var self = new events.EventEmitter();
  // Because HTTP transport consists of multiple exchanges, an universally unique identifier is
  // required. FYI, it has nothing to do with a socket's id.
  self.id = uuid.v4();
...
// The client performs a HTTP persistent connection and watches changes in response and the
// server prints chunk as data to response.
function createStreamTransport(req, res) {
  ...
  // The content-type headers should be `text/event-stream` for Server-Sent Events and
  // `text/plain` for others. Also the response should be encoded in `utf-8`.
  res.setHeader("content-type", "text/" +
      // If it's Server-Sent Events, `cettia-transport-sse` param is `true`.
    (req.params["cettia-transport-sse"] === "true" ? "event-stream" : "plain") + "; charset=utf-8");
  // The padding is required, which makes the client-side transport on old browsers be aware of
...
// The client performs a HTTP persistent connection and the server ends the response with data.
// Then, the client receives it and performs a request again and again.
