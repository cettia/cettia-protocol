...
module.exports = function(uri, options) {
  ...
  var queue = [];
  // For the client to send message to the server. In order to guarantee message is sent in
  // order, the above `sending` and `queue` are used.
  self.write = function(data) {
    ...
    function send(data) {
      ...
      reqOpts.headers = {};
      // `id` param should be added to query. As it has already `transport` param, `&` can be
      // preceded safely.
  ...
  self.close = function() {
    ...
    // of this connection to make sure. In this request, `id` param should be added to query and
    // `when` param should be set to `abort`.
