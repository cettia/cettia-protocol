...
function createSocket(options) {
    ...
    self.handshake = function(t) {
        function handshake() {
            ...
            // Aggregates id, heartbeat and _heartbeat, makes URI from them,
            // sends it as the first message of transport. A client socket will
            // fire `open` event.
        ...
        if (self.state === "opened") {
            // If so, closes the transport to make sure there is no connection
            // leak and make the handshake to be performed on `close` event.
    ...
    // heartbaet event periodically.
    self.on("heartbeat", function() {
        // Then, cancels the timer, sets it up again and sends the heartbeat
        // event as a response.
