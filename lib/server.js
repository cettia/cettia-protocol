...
module.exports = function() {
  ...
  var options = {
    ...
    heartbeat: 20000,
    // This is just to speed up heartbeat test and not required generally. It means the time to
    // wait for the server's response. The default value is `5000`.
    _heartbeat: 5000
  };
  ...
  self.handle = function(transport) {
    ...
    // If it attaches a socket id, looks up a socket whose id is the given id from the repository.
    if (headers.sid) {
...
function createSocket(options) {
  ...
  self.handshake = function(t) {
    function handshake() {
      ...
      // Aggregates id, heartbeat and _heartbeat, makes URI from them, sends it as the first
      // message of transport. A client socket will fire `open` event.
  ...
  // The client will start to heartbeat on its `open` event and send the heartbaet event
  // periodically.
  ...
  self.on("close", function() {
    ...
    // established again again within 1 min or `deleted` state if not.
    self.state = "closed";
  ...
  self.send = function(type, data, resolved, rejected) {
    ...
    if (self.state !== "opened") {
      ...
      // Convert the event to a JSON message and sends it through the transport.
