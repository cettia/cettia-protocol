...
module.exports = function(uri, options) {
    ...
    self.on("connecting", function() {
        ...
        // A reference of temporary transport to be used while finding working
        // transport.
        var trans;
        // Associates the working transport with the socket.
        function init(trans) {
            // Assign `trans`, a variable for the working transport, to
            // `transport`, a variable for the transport associated with the
            // socket.
            transport = trans;
        ...
        // Tries a connection with next URI.
        function find() {
            ...
            for (var i = 0; i < options.transports.length; i++) {
                ...
                if (trans) {
                    ...
                    trans.open();
                    // If it fails, it should try with the next URI.
                    trans.on("close", find);
                    // Node.js kills the process by default when an `error`
                    // event is fired if there is no listener for it. However, a
                    // user doesn't need to be notified of an error from this
                    // transport until it is recognized as a working transport
                    // as it is a part of a process to find a working transport.
                    trans.on("error", function() {});
                    // If it succeeds, a process to find working transport is
                    // terminated. The first transport text message is used to
                    // handshake the socket protocol. `once` registers one-time
                    // event handler.
                    trans.once("text", function(text) {
                        // The handshake output is in the form of URI and uses
                        // query part to represent protocol header.
                        ...
                        // handshaking is completed, it removes `close`
                        // event's `find` handler,
                        trans.removeListener("close", find);
                        // initializes the transport
                        init(trans);
                        // and fires `open` event.
                        self.emit("open");
                    });
                    break;
                }
        ...
        // TODO dealing with stop function must be done after trans is assigned.
        // This is to stop the whole process to find a working transport when
        // the `close` method is called before `open` event.
        function stop() {
            trans.removeListener("close", find);
            trans.close();
        }
        ...
        // If the process ends, `stop` function should be removed.
