...
module.exports = function(uri, options) {
    ...
    self.on("close", function() {
        ...
        if (options.reconnect) {
            ...
            // `waiting` event.
            setTimeout(function() {
                // Increments a total number of reconnection attempts.
                reconnectTry = reconnectTry ? reconnectTry + 1 : 1;
                // Finds a reconnection delay by executing `reconnect` option.
                reconnectDelay = options.reconnect.call(self, reconnectDelay, reconnectTry);
                // If the returned value is not `false`, sets a reconnection
                // timer and fires `waiting` event.
