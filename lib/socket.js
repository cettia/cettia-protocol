...
module.exports = function(uri, options) {
    ...
    // `connecting`, `opened`, `closed` and `waiting`.
    self.state = null;
    ...
    // A function to determine whether or not to reconnect. It is called every
    // time after the `close` event with the last delay in milliseconds used or
    // null at first and the total number of reconnection attempts. It should
    // return a reconnection delay in milliseconds. A boolean value of `false`
    // or a function returning `false` stand for no reconnection.
    ...
    self.open = function() {
        ...
        transport = null;
        // Because a user might execute this method by force for some reason, it
        // shouldn't conflict with the already scheduled reconnection. It
        // cancels the reconnection timer.
        clearTimeout(reconnectTimer);
        // Fires the `connecting` event that is the first socket event that
        // user can handle after a little while to give a user a chance to add
    ...
    // Closes the socket.
    self.close = function() {
        // Prevents further reconnection by setting `reconnect` option to
        // `false`.
        ...
        if (self.state === "connecting") {
            // The `stop` function declared in the below `connecting` event
            // handler should run. As it is registered as `close` event handler,
            // firing `close` event will execute `stop` function.
            self.emit("close");
    ...
    // Finds a working transport and establishes a connection on `connecting`
    // event.
    self.on("connecting", function() {
        // Transition to `connecting` state. It transitions to `opened` state if
        // a connection is established successfully or `closed` state if a
        // connection couldn't be established.
        self.state = "connecting";
        // If `uri` is a string, makes it array. FYI, `[].slice.call(uri)`
        // returns a copied array of `uri`.
        var uris = Array.isArray(uri) ? [].slice.call(uri) : [uri];
        // Translates abbreviated URIs into normal URIs and attaches `id` if it
        // exits. Then, the manipulated `uris`'s each element corresponds to
        // each transport.
        for (var i = 0; i < uris.length; i++) {
            var urlObj = url.parse(uris[i], true);
            delete urlObj.search;
            // If the socket id exists, attaches it to query under the name of
            // `sid`.
            if (id) {
                urlObj.query.sid = id;
            }
        ...
        function find() {
            ...
            // it means that all tries failed.
            if (uris.length === 0) {
                // Now that `connecting` event is being fired, if `error` and
                // `close` event is fired, user's `error` and `close` event
                // handlers are always prior to user's `connecting` event
                // handler. As it is nonsense, delay to fire them a little
                // while.
                setTimeout(function() {
            ...
            var trans;
            // Get the first uri by removing it from `uris`. For example,
            // `[1,2].shift()` returns `1` and make the array `[2]`.
            var uri = uris.shift();
            // Because transport handles only the specific URI, URI determines
            // transport. It finds which transport factory can handle this `uri`
            // among transport factories specified by `transports` option.
            for (var i = 0; i < options.transports.length; i++) {
                // A transport factory creates and returns transport object if
                // it can handle the given URI and nothing if not.
            ...
            // If there is no transport matching with the given URI, try a
            // connection with the next URI.
            if (!trans) {
                find();
                return;
            }
            // This is to stop the whole process to find a working transport
            // when the `close` method is called before `open` event.
            function stop() {
                trans.removeListener("close", find);
                trans.close();
            }
            // Until the socket is opened, `close` method should trigger `stop`
            // function as well.
            self.once("close", stop);
            // it establishes a connection.
            trans.open();
            // If it fails, it should remove stop listener and try a connection
            // with the next URI.
            trans.on("close", function() {
                self.removeListener("close", stop);
            });
            trans.on("close", find);
            // Node.js kills the process by default when an `error` event is
            // fired if there is no listener for it. However, a user doesn't
            // need to be notified of an error from this transport until it is
            // recognized as a working transport as it is a part of a process to
            // find a working transport.
            trans.on("error", function() {});
            // If it succeeds, a process to find working transport is
            // terminated. The first transport text message is used to handshake
            // the socket protocol. Note that `once` registers one-time event
            // handler.
            trans.once("text", function(text) {
                // The handshake output is in the form of URI and uses query
                // part to represent protocol header.
                var headers = url.parse(text, true).query;
                // The server prints an id of the socket as a protocol header
                // under the name of `sid` every time transport connects. If the
                // server issues a new id,
                if (id !== headers.sid) {
                    // That means there was no `id` or the server socket whose
                    // id is `id` was deleted in the server due to long-term
                    // disconnection.
                    id = headers.sid;
                ...
                // Now that the working transport is found and handshaking is
                // completed, it removes `close` event handler `find`.
                trans.removeListener("close", find);
                ...
                transport.on("text", function(text) {
                    ...
                    // * `reply: boolean`: `true` if this event requires the
                    // reply.
                    ...
                    if (!event.reply) {
                        ...
                        // argument and calls the server's `resolved` or
    ...
    self.on("open", function() {
        ...
        function setHeartbeatTimer() {
            ...
            // _heartbeat` milliseconds.
            heartbeatTimer = setTimeout(function() {
                self.send("heartbeat");
                // If the server echoes back the sent `heartbeat` event, clears
                // the timer and set it again.
                self.once("heartbeat", function() {
                    clearTimeout(heartbeatTimer);
                    setHeartbeatTimer();
                });
                // Sets a timer to fire a heartbeat error and close the
                // connection if the server doesn't respond in the `_heartbeat`
                // milliseconds.
                heartbeatTimer = setTimeout(function() {
    ...
    self.on("close", function() {
        // Transition to `closed` state. It transitions to `waiting` state if
        // reconnection is scheduled.
        self.state = "closed";
        if (options.reconnect) {
            // Gives a user a chance to handle `close` event before firing
            // `waiting` event.
            setTimeout(function() {
                // Determines whether to schedule reconnection by providing the
                // last reconnection delay and the total number of reconnection
                // attempts to `reconnect` option.
                reconnectDelay = options.reconnect.call(self, reconnectDelay, reconnectTry);
                // If the returned value is not `false`, schedules a
                // reconnection timer.
                if (reconnectDelay !== false) {
                    ...
                    // Fires `waiting` event with information about scheduled
                    // reconnection - a time to wait for reconnection and the
                    // number of attempts.
    ...
    self.on("waiting", function(delay, attempts) {
        // Transition to `waiting` state. It transitions to `connecting` state
        // after `delay` milliseconds.
        self.state = "waiting";
    });

    // An event id. It should be unique among events to be sent to the server
    // and has nothing to do with one the server sent.
    var eventId = 0;
    ...
    self.send = function(type, data, resolved, rejected) {
        ...
        if (self.state !== "opened") {
            ...
            // Converts the event to a JSON message and sends it through the
            // transport.
